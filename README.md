...
Those two are what the schedule sends. There is a third output if you want it —
the **Weekly Rundown**, the same week as a page you sit down with rather than a
message you glance at. It is generated by hand, in a project, using two extra
files already in this repo. It needs no Twilio, no keys and no schedule, so it
works whether or not you ever finish the setup below. See
`guides/the-weekly-rundown.md`.
...
Open `_shared/`. Five of the files are yours to fill in, and they are
templates, not examples. Every bracket is yours to replace.

`_shared/README.md` is the annotated version: what each file is for, why it
exists, and the two piles every correction sorts into. If you only skim one
thing, skim `sources.md`, because it is the file that decides what this thing
is allowed to read.

The other two files in there, `rundown-template.md` and
`rundown-skeleton.html`, are optional and need no filling in. They are for the
page version, which is a by-hand thing and nothing to do with the scheduled
run. Ignore them for now; `guides/the-weekly-rundown.md` explains them when you
want one.

> **Check:** no square brackets left in any of the five you filled in.
...
|---|---|
| `_shared/` | The factory. Five files, plain English, edited by hand. The rules, the voice, the shape, the household, the sources. Plus two optional ones for the page |
| `guides/` | Running it by hand in a project, with no code at all. And the page version |
